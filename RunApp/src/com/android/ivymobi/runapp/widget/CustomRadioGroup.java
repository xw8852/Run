...
public class CustomRadioGroup extends LinearLayout {
    ...
    public CustomRadioGroup(Context context, AttributeSet attrs) {
        ...
        final int index = attributes.getInt(R.styleable.RadioGroup_orientation, VERTICAL);
        ...
        public void onChildViewAdded(View parent, View child) {
            if (parent == CustomRadioGroup.this && child instanceof RadioButton) {
                ...
                for (int i = 0; i < size; i++) {
                    View _child = group.getChildAt(i);
                    if (_child instanceof RadioButton) {
                        int id = _child.getId();
                        if (id == View.NO_ID) {
                            id = _child.hashCode();
                            _child.setId(id);
                        }
                        ((RadioButton) _child).setOnCheckedChangeListener(mChildOnCheckedChangeListener);
                    }
                }
            }
